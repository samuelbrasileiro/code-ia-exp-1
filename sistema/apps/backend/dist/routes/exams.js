...
function labelForChoice(index, mode) {
    if (mode === "powersOfTwo") {
...
function renderExamPdf(doc, exam, variant, questionMap) {
...
async function generatePdfBuffer(exam, variant, questionMap) {
...
function buildAnswerKeyCsv(exam, variants, questionMap) {
    ...
    variants.forEach((variant) => {
        const answers = variant.questions.map((entry) => {
            ...
            if (exam.answerLabelingMode === "powersOfTwo") {
                const sum = correctLabels.reduce((total, value) => total + Number.parseInt(value, 10), 0);
                return String(sum);
            }
...
router.post("/:id/pdf", async (req, res) => {
    ...
    res.setHeader("Content-Disposition", `attachment; filename="exam-${exam.id}-${variant.variantId}.pdf"`);
    const doc = new PDFDocument({ margin: 50, size: "A4" });
    doc.pipe(res);
    renderExamPdf(doc, exam, variant, questionMap);
    doc.end();
...
router.post("/:id/pdf-zip", async (req, res) => {
    ...
    const pdfBuffers = await Promise.all(variants.map((variant) => generatePdfBuffer(exam, variant, questionMap)));
...
router.post("/:id/answer-key", async (req, res) => {
    ...
    res.setHeader("Content-Disposition", `attachment; filename="answer-key-${variant.examId}-${variant.variantId}.csv"`);
    res.send(csvContent);
});
